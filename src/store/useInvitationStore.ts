...
export interface InvitationState {
  ...
  setGroomParents: (
    type: 'father' | 'mother',
    data: { name?: string; isDeceased?: boolean; isHidden?: boolean }
  ) => void;
  setBrideParents: (
    type: 'father' | 'mother',
    data: { name?: string; isDeceased?: boolean; isHidden?: boolean }
  ...
  setGallery: (
    images:
      ...
      | ((prev: { id: string; url: string }[]) => { id: string; url: string }[])
  ) => void;
  ...
  setAccounts: (
    accounts:
      | InvitationState['accounts']
      | ((prev: InvitationState['accounts']) => InvitationState['accounts'])
  ) => void;
...
export type InvitationData = {
  [K in keyof InvitationState as InvitationState[K] extends (...args: unknown[]) => unknown
    ? never
    : K]: InvitationState[K];
};
...
export const INITIAL_STATE = {
  ...
  message:
    '<p style="text-align: center">곁에 있을 때 가장 나다운 모습이 되게 하는 사람<br>꿈을 꾸게 하고 그 꿈을 함께 나누는 사람<br>그런 사람을 만나 이제 하나가 되려고 해요.<br><br>저희의 뜻깊은 시작을 함께 나누어 주시고<br>따뜻한 마음으로 축복해 주시면 감사하겠어요.</p>',
  ...
  accountsDescription:
    '축하의 마음을 담아 축의금을 전달하고자 하시는 분들을 위해\n계좌번호를 안내해 드려요.',
  ...
  closing: {
    ...
    content:
      '<p style="text-align: center">서로가 마주보며 다져온 사랑을<br>이제 함께 한 곳을 바라보며 걸어가려고 해요.<br>저희의 새 출발을 축복해 주세요.</p>',
  },
  isApproved: false,
...
export const useInvitationStore = create<InvitationState>()((set) => ({
  ...
  setGallery: (images) =>
    set((state) => ({ gallery: typeof images === 'function' ? images(state.gallery) : images })),
