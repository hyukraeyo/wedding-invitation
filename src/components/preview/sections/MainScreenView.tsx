...
const MainScreenView = memo(({
    ...
    const isBasicLayout = ['classic', 'minimal', 'english', 'korean', 'basic'].includes(mainScreen.layout);
            ...
            <div className={clsx(styles.content, isFillLayout ? styles.centerFill : styles.pt10) || ''}>
                ...
                <div className={clsx(styles.headerArea, isBasicLayout ? styles.headerVisible : styles.headerHidden) || ''}>
                    ...
                    {mainScreen.layout === 'minimal' ? (
                        <>
                            <div className={clsx(styles.minimalDateRow) || ''} style={{ fontSize: 'calc(28px * var(--font-scale))' }}>
                                {date ? (
                                    (() => {
                                        const d = new Date(date);
                                        return (
                                            <>
                                                <span className={clsx(styles.minimalDateBox) || ''}>{d.getFullYear()}</span>
                                                <span className={clsx(styles.minimalDateBox) || ''}>{String(d.getMonth() + 1).padStart(2, '0')}</span>
                                                <span className={clsx(styles.minimalDateBox) || ''}>{String(d.getDate()).padStart(2, '0')}</span>
                                            </>
                                        );
                                    })()
                                ) : null}
                            </div>
                            <div className={clsx(styles.weekdayText) || ''} style={{ fontSize: 'calc(14px * var(--font-scale))' }}>
                                            ...
                                            mainScreen.expandPhoto ? '0px' :
                                                mainScreen.layout === 'basic' ? `20px 20px ${mainScreen.effect === 'mist' ? '0px 0px' : '20px 20px'}` :
                                                    mainScreen.layout === 'frame' ? `4px 4px ${mainScreen.effect === 'mist' ? '0px 0px' : '4px 4px'}` :
                                                        mainScreen.layout === 'fill' ? '0px' : `20px 20px ${mainScreen.effect === 'mist' ? '0px 0px' : '20px 20px'}`
                    }}
                >
                    {imageUrl ? (
                        ...
                        <>
                            ...
                            {(groom.firstName || bride.firstName) ? (
                                ...
                                >
                                    ...
                                    <span className={clsx(styles.connector) || ''} style={{ fontSize: 'calc(16px * var(--font-scale))' }}>
