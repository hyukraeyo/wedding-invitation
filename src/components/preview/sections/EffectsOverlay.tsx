  ...
  <svg width="0" height="0" className={styles.hiddenSvg}>
    <defs>
      ...
      <linearGradient id="gradient-pink" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop offset="0%" stopColor={PALETTE.ROSE_100} />
        <stop offset="100%" stopColor={PALETTE.ROSE_200} />
      </linearGradient>
    </defs>
  </svg>
);
...
const Variant1 = () => (
  <svg viewBox="0 0 64 64" className={styles.svgContainer}>
    <g transform="translate(10, 0) scale(0.7, 1)">
      <path
        d="M10 32C10 18 26 6 42 6C58 6 62 22 62 34C62 50 46 62 30 62C14 62 10 46 10 32Z"
        fill={PALETTE.ROSE_200}
        opacity="0.7"
      />
      <path
        d="M32 10C40 10 54 24 54 36C54 48 40 58 32 58C24 58 10 48 10 36C10 24 24 10 32 10Z"
...
const Variant2 = () => (
  <svg viewBox="0 0 64 64" className={styles.svgContainer}>
    <g transform="scale(0.5333)">
      <path
        d="M62 12 C48 24, 34 44, 34 64 C34 84, 48 103, 62 108 C74 102, 92 84, 92 60 C92 40, 76 22, 62 12 Z"
        fill={PALETTE.ROSE_200}
      />
      <path
        d="M60 18 C50 28, 40 46, 40 64 C40 80, 50 96, 60 104"
        stroke={PALETTE.ROSE_100}
        strokeWidth="3"
        strokeLinecap="round"
        opacity="0.8"
        fill="none"
  ...
  <svg viewBox="0 0 140 220" className={styles.svgContainer}>
    <defs>
      <radialGradient id="grad-new-petal" cx="45%" cy="35%" r="75%">
        ...
        <stop offset="100%" stopColor={PALETTE.ROSE_400} />
      </radialGradient>
    </defs>
    <path
      d="M 70 220 Q 15 160 35 70 Q 55 15 95 35 Q 140 75 120 160 Q 100 210 70 220 Z"
      fill="url(#grad-new-petal)"
    />
  </svg>
);
...
const Variant4 = () => (
  <svg viewBox="0 0 120 180" className={styles.svgContainer}>
    <path
      d="M78 8 C99 20 112 48 109 82 C106 118 82 156 52 170 C26 182 10 160 12 124 C15 78 38 28 60 12 C68 6 74 6 78 8 Z"
      fill={PALETTE.ROSE_100}
    />
  </svg>
);

const PETAL_COLORS = [PALETTE.ROSE_50, PALETTE.ROSE_100, PALETTE.ROSE_200, PALETTE.ROSE_400];

const EffectsOverlay = memo(({ effect, effectOnlyOnMain }: EffectsOverlayProps) => {
      ...
      for (let i = 0; i < count; i++) {
        if (effect === 'snow') {
          ...
          newParticles.push({
            ...
            animationClass: [
              styles.animateFallSwayLeft,
              styles.animateFallSwayRight,
              styles.animateFallFlutter,
              styles.animateFallSpiral,
      ...
      <div className={clsx(styles.inner, effectOnlyOnMain ? styles.hFull : styles.hScreen)}>
        ...
        {effect === 'snow'
          ? particles.map((p) => (
              <div
                key={p.id}
                className={clsx(styles.snowParticle, styles.animateFall)}
                style={p.style}
              />
            ))
          : null}
        ...
        {effect === 'cherry-blossom'
          ? particles.map((p) => (
              <div
                key={p.id}
                className={clsx(
                  styles.cherryParticle,
                  p.animationClass || styles.animateFallSwayLeft
                )}
                style={p.style}
              >
