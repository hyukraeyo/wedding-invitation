...
export default function NotificationsClient({ userId, notifications }: NotificationsClientProps) {
  ...
  const processedNotifications = useMemo(() => {
    return notifications.map((notif) => {
      ...
      const StatusIcon = parsed.isApproved
        ? CheckCircle2
        : parsed.isRevoked
          ? XCircle
          : AlertCircle;
  ...
  useEffect(() => {
    const clearNotifications = async () => {
      ...
      const currentIdsHash = notifications
        .map((n) => n.id)
        .sort()
        .join(',');
                  ...
                  >
                    <div className={styles.itemHeader}>
                      ...
                      <div className={styles.itemInfo}>
                        ...
                        <div className={styles.itemDate}>{notif.formattedDate}</div>
                      </div>
                      <div
                        className={clsx(
                          styles.chevronWrapper,
                          isExpanded && styles.expandedChevron
                        )}
                      >
                      ...
                      <div className={styles.reasonBox}>
                        ...
                        <div
                          ...
                          dangerouslySetInnerHTML={{
                            __html: notif.displayReason || '상세 사유가 없어요.',
                          }}
                        />
                      </div>
                    </div>
